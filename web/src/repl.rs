...
use xterm_js_rs::{
    ...
    FontWeight, Terminal, TerminalOptions, Theme,
};
        ...
        match data.as_str() {
            ...
            terminal_sequences::CTRL_LEFT => {
                if ss.cursor.cur_cursor() > 0 {
                    let mut word_bound_index = ss.line.split_word_bound_indices().rev().peekable();
            ...
            terminal_sequences::CTRL_RIGHT => {
                if ss.cursor.cur_cursor() < ss.line.len() {
                    let mut word_bound_index = ss.line.split_word_bound_indices().peekable();
            ...
            terminal_sequences::BS | "\u{7f}" => {
                ...
                if let (true, Ok(true), Ok(Some(prev_boundary))) = (
                    ...
                    ss.line
                        .replace_range(prev_boundary..ss.cursor.cur_cursor(), "");
            ...
            terminal_sequences::DELETE => {
                ...
                if let (true, Ok(true), Ok(Some(next_boundary))) = (
                    ...
                    ss.line
                        .replace_range(ss.cursor.cur_cursor()..next_boundary, "");
                    ss.cursor = GraphemeCursor::new(ss.cursor.cur_cursor(), ss.line.len(), true);
                    self.save_history();
                }
            }
            ...
            _ => {
                ...
                ss.cursor =
                    GraphemeCursor::new(ss.cursor.cur_cursor() + data.len(), ss.line.len(), true);
